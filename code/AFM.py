...
class AFM(BaseEstimator, TransformerMixin):
    def __init__(self, features_M, pretrain_flag, save_file, attention, hidden_factor, valid_dimension,
                 ...
                 freeze_fm, epoch, batch_size, learning_rate, lamda_attention, keep, optimizer_type, batch_norm, decay,
                 verbose, micro_level_analysis, random_seed=2016):
    ...
    def _init_graph(self):
        ...
        with self.graph.as_default():  # , tf.device('/cpu:0'):
            ...
            if self.attention:
                element_wise_product = tf.reshape(self.element_wise_product, shape=[-1, self.hidden_factor[1]])
                attention_mul = tf.matmul(element_wise_product, self.weights['attention_W'])

                self.attention_mul = tf.reshape(attention_mul, shape=[-1, num_interactions, self.hidden_factor[0]])
    ...
    def _initialize_weights(self):
        ...
        if self.pretrain_flag > 0 or self.micro_level_analysis:
            ...
            # feature_embeddings = pretrain_graph.get_tensor_by_name('feature_embeddings')
...
def make_save_file(args):
    ...
    # pretrain_path = '../data/%s_%d' %(args.dataset, eval(args.hidden_factor)[1])
    if args.mla:
        pretrain_path += '_mla'
...
def train(args):
    ...
    model = AFM(data.features_M, args.pretrain, save_file, args.attention, eval(args.hidden_factor), args.valid_dimen,
                ...
                args.lamda_attention, eval(args.keep), args.optimizer,
